...
public class DAO {
	...
	public static final String TABLE_DDL = "create table " + TABLE_GEOBOOKMARKS
			+ "(" + COLUMN_ID + " integer primary key autoincrement," + ""
			+ COLUMN_NAME + " text," + "" + COLUMN_DESCRIPTION + " text," + ""
			+ COLUMN_OFFSETX + " integer," + "" + COLUMN_OFFSETY + " integer,"
			+ "" + COLUMN_S + " integer," + "" + COLUMN_X + " integer," + ""
			+ COLUMN_Y + " integer," + "" + COLUMN_Z + " integer" + ");";
	...
	public void saveGeoBookmark(GeoBookmark bookmark) {
		if (bookmark.getId() != -1) {
			...
			initialValues
					.put(DAO.COLUMN_DESCRIPTION, bookmark.getDescription());
	...
	private void updateGeoBookmark(GeoBookmark bookmark) {
		...
		db.update(TABLE_GEOBOOKMARKS, args, COLUMN_ID + "=" + bookmark.getId(),
				null);
	}
	...
	public List<GeoBookmark> getBookmarks() {
		...
		try {
			...
			for (int i = 0; i < numRows; ++i) {
				...
				bookmark.setDescription(c.getString(c
				...
				RawTile tile = new RawTile(
						c.getInt(c.getColumnIndex(COLUMN_X)), c.getInt(c
								.getColumnIndex(COLUMN_Y)), c.getInt(c
								.getColumnIndex(COLUMN_Z)), c.getInt(c
								.getColumnIndex(COLUMN_S)));
